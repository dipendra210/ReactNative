...
from .serializers import UserSerializer, CarBrandSerializer, RegisterSerializer, PostSerializer, PostSerializer4Regular, CarModelSerializer, UserRequestSerializer, PostSerializer4Special
...
class SpecialPost(APIView):
    def post(self, request, format=None):
        serializer1 = PostSerializer4Regular(data=request.data)
        print(request.data)
        ...
        if serializer1.is_valid():
            ...
            new_data                      = {}
            new_data['post_id']           = instance.id
            new_data['first_owner']       = request.data.get('first_owner')
            new_data['is_from_kwait']     = request.data.get('is_from_kwait')
            new_data['is_under_warranty'] = request.data.get('is_under_warranty')
            new_data['is_get_accident']   = request.data.get('is_get_accident')
            new_data['is_repainted']      = request.data.get('is_repainted')
            new_data['condition']         = request.data.get('condition')
            new_data['is_accept_exam']    = request.data.get('is_accept_exam')
            new_data['mini_price']        = request.data.get('mini_price')
            new_data['has_visible_defect']= request.data.get('has_visible_defect')
            new_data['comment']           = request.data.get('comment')
            new_data['communication']     = request.data.get('communication')

            print(new_data)
            serializer2 = PostSerializer4Special(data=new_data)
...
# SUBMIT POSTING
class RegularPost(APIView):
    def post(self, request, format=None):
        serializer = PostSerializer4Regular(data=request.data)
        print(request.data)
