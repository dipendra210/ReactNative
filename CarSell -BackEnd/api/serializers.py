...
from .models import Brand, Post, SpecialPost, Model, UserRequest, Profile
...
class PostSerializer4Regular(serializers.ModelSerializer):
    user  = serializers.PrimaryKeyRelatedField(queryset=User.objects.all())
    brand = serializers.PrimaryKeyRelatedField(queryset=Brand.objects.all())
    model = serializers.PrimaryKeyRelatedField(queryset=Model.objects.all())
...
class PostSerializer4Special(serializers.ModelSerializer):
    ...
    class Meta:
        model = SpecialPost
        fields = ['post', 'first_owner', 'is_from_kwait', 'is_under_warranty', 'is_get_accident', 'is_repainted', 'condition', 'is_accept_exam', 'mini_price', 'has_visible_defect', 'comment', 'communication']
